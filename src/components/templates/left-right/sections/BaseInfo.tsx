        ...
        <SectionWrapper sectionId="basic">
            <div className={styles.container}>
                <div className={styles.leftContent}>
                    ...
                    <div className={cn("flex flex-col", styles.nameTitle)}>
                        {nameField.visible !== false && basic[nameField.key] && (
                            <motion.h1 layout="position" className="font-bold whitespace-pre-wrap break-keep text-balance" style={{ fontSize: "30px" }}>{basic[nameField.key] as string}</motion.h1>
                        )}
                        {titleField.visible !== false && basic[titleField.key] && (
                            <motion.h2 layout="position" className="whitespace-pre-wrap break-keep text-balance" style={{ fontSize: "18px" }}>{basic[titleField.key] as string}</motion.h2>
                        )}
                    </div>
                </div>
                <motion.div layout="position" className={styles.fields} style={{ fontSize: `${globalSettings?.baseFontSize || 14}px`, color: "rgb(75, 85, 99)", maxWidth: layout === "center" ? "none" : "600px" }}>
                    {allFields.map((item) => (
                        <motion.div key={item.key} className="flex items-center overflow-hidden text-baseFont">
                            {useIconMode ? (
                                <div className="flex items-center gap-1">
                                    {getIcon(item.icon)}
                                    {item.key === "email" ? <a href={`mailto:${item.value}`} className="underline break-all">{item.value}</a> : <span className="break-all">{item.value}</span>}
                                </div>
                            ) : (
                                <div className="flex items-center gap-2 overflow-hidden">
                                    {!item.custom && <span className="shrink-0">{t(`basicPanel.basicFields.${item.key}`)}:</span>}
                                    {item.custom && <span className="shrink-0">{item.label}:</span>}
                                    <span className="break-all" suppressHydrationWarning>{item.value}</span>
                                </div>
                            )}
                        </motion.div>
                    ))}
